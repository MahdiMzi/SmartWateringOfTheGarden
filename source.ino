...
#define wateringWarning_S 180 // 86400s == 24h
...
void wateringWarningTimerHandler()
{
  ...
  wateringWarningTimer.attach(wateringWarning_S, wateringWarningTimerHandler);
  //(&& digitalRead(ElectricalValve_PIN) == HIGH) For more stability
  if (status == LOW_VAL && isSmart == 1) 
  {
    digitalWrite(ElectricalValve_PIN, LOW);
    sendSMS(ownersPhoneNumber, "Watering warning !! and Start watering...");
    delay(500);
  }
  else if (status == LOW_VAL)
  {
...
void samplingTimerHandler()
{
  if (samplingCounter >= samplingRate)
  {
    ...
    if (status == HIGH_VAL && digitalRead(ElectricalValve_PIN) == LOW)
    {
      digitalWrite(ElectricalValve_PIN, HIGH);
      sendSMS(ownersPhoneNumber, "Stop watering...");
      delay(500);
